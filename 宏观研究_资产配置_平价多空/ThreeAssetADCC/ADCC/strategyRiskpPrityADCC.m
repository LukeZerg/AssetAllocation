function [ Position,CloseData, theWeights ] = strategyRiskpPrityADCC( ...
startday,endday, backtime, capital, Position0, CloseData0,Information, ...
    names, cashcol, alpha, backtimeD, cycle, FactorData)
%风险平价策略
% startday = '2013-02-04';
% endday = '2017-02-03';
% backtime = 60;
...
%生成宏观因子择时周序列
transvectorw = computeFreqSubscript(tradingdays1, 'w');%找周调仓下标
dayfind = tradingdays1(transvectorw);%找出调仓日期
FactorDataWeek = [];%周数据
ndayfind = size(dayfind,1);
%按照调仓日期dayfind得到周序列
for i = 1:ndayfind
    jfind = FindinCell(FactorData,1,dayfind{i});
    FactorDataWeek = [FactorDataWeek;FactorData(jfind,:)];
end
...
for j = 1:n
    if onePosition{j+1,6} %如果是调仓日
        ...
        %择时数据跟权重数据计算独立开来
        direction = getDirectionLLT(subLLT,cashcol); %利用LLT择时判断
        for iPosition = 1:m
