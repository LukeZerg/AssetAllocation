function [ Position,CloseData, theWeights ] = strategyRiskpPrityLLT_LSRisk( ...
startday,endday, backtime, capital, Position0, CloseData0,Information, ...
    names, cashcol, alpha, backtimeD, cycle)
%风险平价策略
% startday = '2013-02-04';
% endday = '2017-02-03';
% backtime = 60;
...
for j = 1:n
    if onePosition{j+1,6} %如果是调仓日
        ...
        %根据direction筛选反向的ret，然后做平价
        sub_anti_direction = sub;%初始化单向风险
        for iPosition = 1:m
            temp = sub_anti_direction(:,iPosition);%取第iPosition资产
            if direction(iPosition) > 0
                temp(temp>0)=0;%只保留小于0的收益率
            elseif direction(iPosition) < 0
                temp(temp<0)=0;%只保留大于0的收益率
            end
            sub_anti_direction(:,iPosition) = temp;
        end
        %因为只看反向风险，所以sub_anti_direction的ret处理成全部为负数更加合理
        sub_anti_direction = abs(sub_anti_direction);
        sub_anti_direction = -1*sub_anti_direction;
        weights = RiskParity(sub_anti_direction);%计算权重
        for iPosition = 1:m
