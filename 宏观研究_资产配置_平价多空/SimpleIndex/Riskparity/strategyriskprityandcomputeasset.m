function [ pnldata, Assetall, Cumsums, weights ] = strategyriskprityandcomputeasset( startday, ...
                            endday, backtime, capital, data, names, cashcol, alpha, backtimeD)
%风险平价策略
% startday = '2013-02-04';
% endday = '2017-02-03';
% backtime = 60;
...
for idata = 1:size(data,2)
    ...
    prices = cell2mat(onedata(2:end,3));
    %LLTs矩阵赋值
    if idata ~= cashcol
        LLTs(:,idata) = priceToLLT(prices,alpha);
    end
    %赋值
    for j = 1:n
...
for j = 1:n
    if onepnldada{j+1,8} %如果是调仓日
        ...
        subLLT = LLTs((j-1-backtime):(j-1-1),:);
        direction = getDirectionLLT(subLLT,cashcol,backtimeD); %利用LLT择时判断
        WandD = weights .* direction; %权重方向之积
        for idata = 1:size(data,2)
            pnldata{1,idata}{j+1,5} = WandD(idata); %Weight
            pnldata{1,idata}{j+1,4} = capital * WandD(idata); %Asset
        end
    end
end
...
%权重统计正数
for i = 1:size(pnldata,2)
    jn = size(pnldata{1,i},1);
    for j = 2:jn
        %weights{j,i+1} = abs(pnldata{1,i}{j,5});%赋值
        weights{j,i+1} = pnldata{1,i}{j,5};%赋值
    end
end
