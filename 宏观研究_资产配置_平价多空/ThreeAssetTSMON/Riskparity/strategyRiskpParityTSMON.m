...
if dayscount < backtimeTSMON
    error('策略起始日期与原始数据起始日期距离小于TSMON策略回溯时间backtimeTSMON');
end
...
%% 计算ret用于风险平价计算，计算exRet超额收益矩阵，标记调仓日 
%初始化
ret = zeros(n, m); %收益率矩阵
exRet = zeros(n, m); %超额收益矩阵
nav_riskfree = cell2mat(riskFreeAsset(2:end,3)); %逆回购利率换算的净值
if size(nav_riskfree,1) ~= n
    error('大类资产与逆回购的数据长度不一致');
end
...
for iPosition = 1:size(Position,2)
    ...
    temp = oneExRet - ret_riskfree;%超额收益
    lentemp = size(temp,1);
    %------------------------------------
    colExRet = [zeros(n - lentemp,1);temp]; %在计算出的超额收益率前补齐0
    exRet(:,iPosition) = colExRet; %对超额收益其中一列赋值
    %------------------------------------
...
for j = 1:n
    if onePosition{j+1,6} %如果是调仓日
        ...
        onedayexRet = exRet(j,:);
        %择时数据跟权重数据计算独立开来
        direction = getDirectionTSMON(onedayexRet,cashcol); %利用TSMON择时判断
        for iPosition = 1:m
