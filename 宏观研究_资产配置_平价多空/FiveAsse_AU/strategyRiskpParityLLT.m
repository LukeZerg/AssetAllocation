...
for j = 1:n
    if onePosition{j+1,6} %如果是调仓日
        ...
        direction = zeros(1,m);
        for iPosition = 1:m
            subLLT = LLTs((j-2-backtimeD(iPosition)):(j-1-1),iPosition); %选择该资产判断斜率的那一段LLT，长度为backtimeD(iPosition)
            %择时数据跟权重数据计算独立开来
            direction(iPosition) = getDirectionLLTsomelongPar(subLLT,longcol,iPosition); %利用LLT择时判断
        end
