function [ Position,CloseData, theWeights, theDirection ] = strategyRiskpParityLLTAsset( ...
startday,endday, backtime, Position0, CloseData0, ...
    names, longcol, alpha, backtimeD, cdata)
%风险平价策略
% startday = '2013-02-04';
% endday = '2017-02-03';
% backtime = 60;
...
theDirection = cell(n+1,m+1);
theDirection(1,:) = [{'TradingDay'},names];
theDirection(2:end,1) = onePosition(2:end,1);
theDirection(2:end,2:end) = {0};
...
%             end
            ...
            %记录方向
            theDirection{j+1, iPosition+1} = direction(iPosition);
        end
    end
end
...
%填充position的curren 3列和direction 4列
 ...
 for k = 1:m %循环多空品种
  ...
  %填充theDirection
for k = 1:m %循环多空品种
     nD = size(theDirection,1);%此品种数据大小
     oneDrect = 0;
     for i = 2:nD
         if Position{1,k}{i,6} == 1 %如果这一天持仓变化了isChg
             %记录下curren和direction
             oneDrect = theDirection{i,k+1};
         end
         theDirection{i,k+1} = oneDrect;
     end
 end
...
theDirection([startnull,endnull],:) = [];
end
