function []= myplot_FourAsset(AssetAll,AssetDataMerge,theWeights)
%净值曲线图
...
xlabel('资产比例配置图')
datetick('x','yy-mm','keepticks');
legend('A股','商品期货','港股', '国债');
grid on;

%累计收益图
m = size(AssetDataMerge,2);
n = size(AssetDataMerge{1,1},1)-1;
cumsums = zeros(n,m);
for i = 1:m
    cumsum_pnlh = cumsum(cell2mat(AssetDataMerge{1,i}(2:end,3)));
    cumsum_pnlt = cumsum(cell2mat(AssetDataMerge{1,i}(2:end,4)));
    cumsums(:,i) = cumsum_pnlh + cumsum_pnlt;
end
subplot(2,2,3),plot(T,cumsums(:,1),T,cumsums(:,2),T,cumsums(:,3),T,cumsums(:,4))
xlabel('累计收益图')
datetick('x','yy-mm','keepticks');
legend('A股','港股', '国债','商品期货');
grid on;
end