function [AssetData, AssetAll] = computeAsset(Position,TradeRecord, Close_strategy, Information)
%从交易记录和价格数据里获取资产信息
...
for i = 1:nPosition %循环品种多空
    ...
    %oneClose_strategy = Close_strategy{1,i}; %该品种Close_strategy数据
    ...
    for j = 1:nt
       ...
       AssetData{1,i}(j+1,8) = {1};%保证金率 deposit 
    end
end
...
for i = 1:nPosition
    ...
    for t = 2:nt  %循环日期,从第二天开始算
        ...
        % Hoding Pnl      
        AssetData{1,i}{t+1,3} = ...     
        double( cdata{t+1,10} - cdata{t,6} )...%昨日合约今日价格-昨日合约昨日价格
        * double(pos{t,3} * pos{t,4}...        %昨日仓位，昨日方向
        * Multiplier{i});                
        %deposit
        %今日仓位=乘数*当日合约当日价格*当日仓位
        todayPosition = double(Multiplier{i} * cdata{t+1,6} * pos{t+1,3});
