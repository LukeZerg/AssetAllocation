function data = getETFdata(startday, endday, contract)
...
for i = 1:nContract
    ...
    [w_wsd_data,~,~,w_wsd_times,~,~]=w.wsd(contract{i},'open,high,low,close,volume,oi',startday,endday,'Fill=Previous');%,volume,position
    ...
    for j = 1:nt
        ...
        data{1,i}{j+1,2} = contract{i};%赋值主力合约
        data{1,i}{j+1,3} = w_wsd_data{j,1};%开盘价格
        data{1,i}{j+1,4} = w_wsd_data{j,2};%最高价
        data{1,i}{j+1,5} = w_wsd_data{j,3};%最低价
        data{1,i}{j+1,6} = w_wsd_data{j,4};%收盘价
        data{1,i}{j+1,7} = w_wsd_data{j,5};%成交量
        data{1,i}{j+1,8} = w_wsd_data{j,6};%持仓量,只有期货有这个属性
    end
    %给前一天合约和前一天合约今日价格赋值
