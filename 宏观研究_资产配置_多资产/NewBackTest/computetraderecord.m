function TradeRecord = computetraderecord(Position,Close_strategy)
%从仓位和价格数据里获取交易记录
    n = size(Position,2);
    ...
    for k = 1:n %分多空品种计算
        ...
        pos = Position{k};
        oneClosetata = Close_strategy{1,k};%该品种价格数据
        ...
        for i = 3:rows %从内容第二行开始
            ...
            %if pos{i,7} == 1;%如果持仓改变日
                ...
                close = oneClosetata{i,6}; %本合约价格
                preclose = oneClosetata{i,10};%前一个合约今日价格
